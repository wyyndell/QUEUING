...
<body>
	 <h1 style="text-align: center;">Office and Client Transaction Report</h1>
        ...
        <form action="#" method="post" id="office">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if (isset($_POST['selected_personnel'])) {
                    ...
                    $sql = "SELECT fname, address, email, purpose, clienttype,
                   arrive, ended,
                   TIMESTAMPDIFF(MINUTE, arrive, ended) AS time_difference_minutes,
                   TIMESTAMPDIFF(HOUR, arrive, ended) AS time_difference_hours
            FROM client_info  WHERE office = '$selectedOffice' AND status IN ('done', 'DONE', 'Done')";
                    $result = $conn->query($sql);
