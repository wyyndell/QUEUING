...
<body>
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['selected_personnel'])) {
            ...
            $sql = "SELECT fname, clientnumber, gender, address, email, purpose, clienttype, arrive, ended,
                    TIMESTAMPDIFF(HOUR, arrive, ended) AS hours_waiting,
                    TIMESTAMPDIFF(MINUTE, arrive, ended) % 60 AS minutes_waiting
                    FROM client_info WHERE office = ? AND status IN ('done', 'DONE', 'Done')";
            ...
            <table>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>' . $row['hours_waiting'] . ' hours ' . $row['minutes_waiting'] . ' minutes</td>';
                        echo '</tr>';
                        $counter++;
                    }
                } else {
                ...
                echo "<tr><td colspan='11'>Please select an office.</td></tr>";
            }
            ?>
            </table>
    </div>
